...
const sections: [string, string, string][] = [
  ...
  ['rehearsal', 'Rehearsal and practice', `<ul>
    ...
    <li><strong>Check what I said</strong> marks each of your lines as <strong>Said</strong>, <strong>Half said</strong> or <strong>Not this line</strong>, so you can see where you dried or jumped. It needs a WhisperX server address under <strong>Settings</strong>, <strong>Advanced</strong>. Your line is sent there after the scene has already carried on, so it never holds the rehearsal up, and the marks last for this run only: nothing is scored, kept or sent anywhere else. It is off until you switch it on.</li>
  ...
  ['settings', 'Settings and paid services', `<ul>
    ...
    <li><strong>Advanced:</strong> press <strong>Show advanced settings</strong> for the Chatterbox server address (when a paid engine reads the parts), the WhisperX server address (used by <strong>Check what I said</strong>; listening itself needs no server), the name shown for this set-up, and <strong>Reuse audio made by an older Script Glow</strong>. Settings are saved in <code>data/connections.json</code>, which never holds a key, so it is safe to copy to another computer.</li>
  </ul>`],
